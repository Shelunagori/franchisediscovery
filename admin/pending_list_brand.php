 ...
 if(isset($_GET['ok']))
{
	...
	if(!empty($where)){
		$query = "select * from brands where status = 'Active' and is_approve='Pending' $where order by id DESC ";
	}else{
		$query = "select * from brands where status = 'Active' and is_approve='Pending' order by id DESC ";
	}
			...
			<div class="col-xs-12">
					  <div class="box">
						<div class="box-header">
						  <h3 class="box-title"><i class="fa fa-fw fa-angle-double-right"></i> Pending Brands List</h3>
						...
						<table class='table table-striped'>
							<tr>
								...
								<td width="20%">
									...
									<?php 
									$sql="SELECT * FROM brands WHERE status='Active' and is_approve='Pending'";
								...
								<tbody>
									...
									<tr role="row" class="odd">
									  <td> <?php echo $sno; ?> </td>	
										...
										<td><?php $registration_id = $row['registration_id'];		
											$queryUser="select * from registration where id=$registration_id ";
											$query_User=mysqli_query($db,$queryUser);
										...
										<td>
											<a title="Approve" class="mb-control1 btn bg-purple btn-rounded btn-sm" href="approve_brand.php?id=<?php echo base64_encode($row['id']); ?>">
												<span class="fa fa-flag-checkered"></span>
											</a>

											<a title="Edit" style="color:#fff;" class="btn btn-info btn-rounded btn-sm" href="brand_edit.php?rowvalue=<?php echo base64_encode($row['id']); ?>">
												<span class="fa fa-edit"></span>
											</a>

											<a title="Delete" class="mb-control1 btn btn-danger btn-rounded btn-sm" onclick="return confirm('Are you sure ?')" href="delete_brand.php?id=<?php echo base64_encode($row['id']); ?>">
