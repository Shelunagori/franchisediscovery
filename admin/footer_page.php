...
$message = '';
	if(isset($_POST['submit']))
	{
		...
		$page_query=mysqli_query($db,"INSERT INTO footer_pages_detail(page_id,detail) VALUES ('$page_id','$message')");
		if($page_query)
		{
			$status = 'success';
	...
	if(isset($_POST['edit']))
	{
		$footer_page_id = $_POST['footer_page_id'];
		$page_id=$_POST['page_id'];
		$message=$_POST['message'];
		$page_query=mysqli_query($db,"update footer_pages_detail set page_id = '$page_id', detail = '$message' where id = '$footer_page_id'");
		if($page_query)
		{
			$status = 'success';
			$message = 'Pages edited successfully !';
		} else {
			$status = 'fail';
			$message = 'Something went wrong !';
		}
	...
	if(@$_GET["Action"] == "Del")
	{
		...
		$sql = "DELETE FROM footer_pages_detail where id = '$id'";
