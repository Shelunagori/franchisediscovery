...
{	$catName = mysqli_real_escape_string($db,$_POST['name']);
	...
	$sql = "INSERT INTO new_categories(name,seo_name,footer_content) VALUES ('$catName','$seo_name','$footer_content')";
...
{	$catName = mysqli_real_escape_string($db,$_POST['name']);
	...
	$seo_name = seo_url($catName);
	$seo_name = $seo_name.'-franchises';
	$sql = "update new_categories set name = '$catName', seo_name = '$seo_name', footer_content = '$footer_content' where id = '$id' and status = 0";
	if ($db->query($sql) === TRUE) {
	...
	if(@$_GET["Action"] == "Del")
	{
		...
		$sql = "update new_categories set status = 1 where id = '$id' and status = 0";
		if ($db->query($sql) === TRUE) {
			$status = 'success';
				...
				<?php $i = 1;
					...
					while($row=mysqli_fetch_array($query)){
						...
						<tr>
							...
							<td>	
								<a style="color:#fff;" class="btn btn-info btn-rounded btn-sm" href="edit_newcategory.php?id=<?php echo base64_encode($row['id']); ?>">
									<span class="fa fa-edit"></span>
								</a>
							</td>
							<td>
								<a class="mb-control1 btn btn-danger btn-rounded btn-sm" onclick="return confirm('Are you sure ?')" href="new_category.php?Action=Del&id=<?php echo base64_encode($row['id']); ?>">
