...
<head>
 ...
 <?php	session_start();
		...
		$query=mysqli_query($db,"SELECT * FROM page_seo where page_id = 21");  
		if($query->num_rows > 0){  
...
<body>
    ...
    <section class="new_arrivals_area  clearfix ">
        <div class="container ">
           ...
           <div class="col-sm-12">
			<?php
				$query=mysqli_query($db,"SELECT * FROM footer_pages_detail where page_id = 22");  
				if($query->num_rows > 0){  
					while($row=mysqli_fetch_array($query)){ 
						echo $row['detail'];
					}
